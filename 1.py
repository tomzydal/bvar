...
def run_cli():
    ...
    print("\n" + "="*6 + " Universal Crypto Hunter (v15.4 - CLI Refactor) " + "="*6)
...
def main():
    ...
    """
    # If no command-line arguments are provided (only the script name), run the interactive CLI.
    if len(sys.argv) == 1:
        run_cli()
        return

    # If any arguments are provided, proceed with command-line parsing.
    parser = argparse.ArgumentParser(
        description="Universal Crypto Hunter - Explore and hunt for crypto keys.",
        formatter_class=argparse.RawTextHelpFormatter,
        epilog="""
Examples:
  # Browse page 100 of Bitcoin puzzle 71
  python %(prog)s --puzzle 71 --page 100

  # Hunt for 10 million random Ethereum keys
  python %(prog)s --hunt --ctype ethereum --random 10000000

  # Hunt a specific key range for legacy bitcoin addresses
  python %(prog)s --hunt --ctype bitcoin_legacy --range 0x1 0x100000
"""
    )
    parser.add_argument('--hunt', action='store_true', help='Start an automated hunt. If not specified, the default action is to browse.')

    # --- Target Selection (required for CLI mode) ---
    ...
    # --- Browsing Options ---
    parser.add_argument('--page', type=int, default=1, help='The page number to start browsing from (browse mode only).')
    parser.add_argument('--pagesize', type=int, default=DISPLAY_SIZE, help='The number of keys per conceptual page (browse mode only).')
    ...
    if args.hunt:
        # --- HUNT MODE ---
        if args.page != 1 or args.pagesize != DISPLAY_SIZE:
             print(f"{C_YELLOW}Warning: --page and --pagesize are for browse mode and will be ignored.{C_RESET}")
        ...
        if args.range:
            ...
            print(f"{C_YELLOW}Error: Hunt mode requires a scope. Use --range, --random, or specify a --puzzle to hunt its full range.{C_RESET}")
            parser.print_help()
            return
        ...
        # --- BROWSE MODE (default) ---
        if args.range or args.random:
            print(f"{C_YELLOW}Warning: --range and --random are for hunt mode. Ignoring for browse mode.{C_RESET}")
